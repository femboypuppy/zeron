...
fn main() -> anyhow::Result<()> {
    ...
    gpui_platform::application().with_assets(icons::Assets).run(move |cx| {
        ...
        theme_library::init(data.clone(), cx); appearance::init(appearance::AppearanceMode::Dark, settings.theme_selection, settings.accent, settings.surface, settings.frost_strength, cx);
